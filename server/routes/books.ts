...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename: (_req, file, cb) => cb(null, `epub-${Date.now()}-${file.originalname}`),
  }),
...
booksRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const parsed = await parseEpub(tmpPath)
      ...
      console.error('[books] error processing EPUB:', err)
...
booksRouter.use((err: Error, _req: Request, res: Response, _next: unknown) => {
  if (err.message === 'Only EPUB files are accepted') {
